...
const checkRoles = async () => {
	try {
		...
		const professorRole = await Role.findOne({
			where: {
				Role_Id: 5,
			},
		});
		if (professorRole) {
			if (professorRole.Role_Name !== 'Professor' || professorRole.Role_Description !== 'Professor role') {
				professorRole.Role_Name = 'Professor';
				professorRole.Role_Description = 'Professor role';
				await professorRole.save();
			}
		} else {
			await addRole('Professor', 'Professor role');
		}
	} catch (err) {
		throw err;
	}
