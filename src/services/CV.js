...
module.exports = async function generateCV(user = {}) {
  ...
  try {
    ...
    const skillsLength = Math.max(
      firstHalfOfSkills.length,
      secondHalfOfSkills.length,
    );

    // download the user's profile picture
    const profilePicture = fs.createWriteStream(
      path.join(__dirname, `../../public/static/${name}.jpg`),
    );
    ...
    if (
      ...
      fs.statSync(path.join(__dirname, `../../public/static/${name}.jpg`))
        .size === 0
    ) {
    ...
    const writeStream = fs.createWriteStream(
      path.join(__dirname, `../../public/static/${name}-CV.pdf`),
    );
    ...
    const DM_Sans_Bold = path.join(
      __dirname,
      '../../public/static/fonts/DMSans-Bold.ttf',
    );
    const DM_Sans_Regular = path.join(
      __dirname,
      '../../public/static/fonts/DMSans-Regular.ttf',
    );
    const DM_Sans_Medium = path.join(
      __dirname,
    ...
    doc
      .image(`public/static/${name}.jpg`, 20, 45, {
        width: 75,
        height: 75,
        align: 'left',
      })
    ...
    const aboutHeight = doc.heightOfString(about, {
      width: 480,
      align: 'justify',
      lineBreak: true,
    });
    ...
    doc
      ...
      .fontSize(12)
      .font(DM_Sans_Regular)
      .list(firstHalfOfSkills, 60, 215 + aboutHeight, {
        bulletRadius: 2,
        textIndent: 10,
        lineGap: 5,
      })
      .list(secondHalfOfSkills, 300, 215 + aboutHeight, {
        bulletRadius: 2,
        textIndent: 10,
        lineGap: 5,
      });
    ...
    doc
      .fontSize(10)
      .text(`Built by Rafiki`, 50, endOfPage, { characterSpacing: 2.5 });
    const cprHeight = doc.heightOfString(`Built by Rafiki`, {
