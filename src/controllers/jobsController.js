...
exports.addJobCategory = async (req, res, next) => {
  try {
    ...
    const Job_Category_exists =
      await job_util.getJobTypeByName(Job_Category_Name);
    if (Job_Category_exists) {
      res
        .status(409)
        .json({
          message: 'Job type already exists',
          Job_Category: Job_Category_exists,
        });
      return;
    }
...
exports.addJobPost = async (req, res, next) => {
  try {
    ...
    if (!Company_Email && !External_Link) {
      res
        ...
        .json({
          ...
          missing_fields: ['Company_Email', 'External_Link'],
        });
      return;
    }
    if (checkMissingFields(required).length > 0) {
      res
        .status(400)
        .json({ message: 'Job Required fields missing', missing_fields });
      return;
    }
    ...
    if (isInternship === true) {
      ...
      if (checkMissingFields(requiredForInternship).length > 0) {
        ...
        res
          .status(400)
          .json({ message: 'Intern Required fields missing', missing_fields });
        return;
      }
    }
    ...
    const formattedDeadline =
      job_post_created.Application_Deadline?.toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
    ...
    const message = `
        ...
        job_post_created.Application_Deadline
    ...
    if (notification) {
      ...
      res
        ...
        .json({
          job_post_created,
          createdSkills,
          messageSentSuccessfully: messageResult,
        });
      return;
    }
...
exports.getJobPosts = async (req, res, next) => {
  try {
    ...
    const job_posts = await job_util.getJobPosts(
      Number(page) || 0,
      Number(limit) || 10,
    );
...
exports.updateJobApplicationStatus = async (req, res, next) => {
  try {
    ...
    const job_application = await job_util.updateJobApplicationStatus(
      Job_Application_Id,
      Status,
    );
    res.status(200).json(job_application);
